<?php
  ...
  $date = date('Y-m-d');
  if(empty($nickname)){
    if(!empty($bNo) && empty($_COOKIE['board_free_' .$bNo.$date])) {
      ...
      if(empty($result)) {
        ...
        setcookie('board_free_' .$bNo.$date, TRUE, time() + (60 * 60 * 24), '/');
      }
    }
  }else if(!empty($nickname)){
    if(!empty($bNo) && empty($_COOKIE['board_free_' .$bNo.$date.$nickname])) {
      $sql = 'update reviewBoard set bbs_hit = bbs_hit + 1 where bbs_no = ' . $bNo;
      $result = $conn->query($sql);
      if(empty($result)) {
        ?>
        ...
        setcookie('board_free_' .$bNo.$date.$nickname, TRUE, time() + (60 * 60 * 24), '/');
      }
    }
  }
...
<br/><br/>
  <div class="col-md-8 col-md-offset-2">
    <article class="boardArticle" border="1" width="100%">
      ...
      <div id="boardView">
        <div id="boardInfo">
          <h3 id="boardTitle"><?php echo '제목 : '.$row['bbs_title']?></h3>
        ...
        <div id="boardContent" class="well"><?php echo $row['bbs_content']?></div>
        <hr/>


        <div class="btnSet" style="float:right">
          <?php
            if($is_login === true && $row['bbs_nickname'] === $nickname){ // 로그인한 회원이 자신의 게시물을 볼 때
          ?>
            <a href="./write.php?bno=<?php echo $bNo?>&amp;page=<?php echo $page?>">수정</a>
